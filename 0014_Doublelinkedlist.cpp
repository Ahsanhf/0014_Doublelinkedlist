...
void addNode()
{
	...
	{ // Step 2: insert the new node at the beginning
		...
		newNode->next = START; //step 3: make new node point to the first node
		if (START != NULL)
		{
...
void deleteNode()
{
	...
	current = START; //step1:start from the first mode
	previous = NULL;
	...
	if (current == NULL)
	{
		cout << "\033[31mThe record with roll nummber " << rollNO << " not found\033[0m" << endl;
		return;
	}
	...
	{ //Node to be deleted is not the first node
		...
		{  //if there's successor, update its prev pointer
...
void revtraverse()
{
	...
	{
		cout << "\nRecords i descending order of roll number are : " << endl;
